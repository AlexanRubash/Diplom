...
export const getDeepinfraTextTranslation = async (task, translate, targetLanguage) => {
  const { data } = await $authHost.post(ROUTES.DEEPINFRA_TRANSLATE_ROUTE, {
    task,
    translate,
    targetLanguage,
  });
console.log(data);
  return data; // Перевод и пояснение
};