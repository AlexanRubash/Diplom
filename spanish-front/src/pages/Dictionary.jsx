...
const Dictionary = observer(() => {
  ...
  const actionDictionaryExport = (rowData) => {
    return (
        <Button
            icon="pi pi-file-export"
  ...
  const handleFileChange = async (event) => {
    try {
      ...
      if (file) {
        ...
        reader.onerror = (e) => {
          showError(`Error reading the file: ${e.target.error.message}`);
        };
        ...
        reader.onload = async (e) => {
          ...
          try {
            ...
            try {
              wordsToImport = JSON.parse(content);
            } catch (jsonError) {
              showError(`Error parsing JSON: ${jsonError.message}`);
              return;
            }

            const existingWords = selectedDictionary?.words || [];

            // Проверка структуры слов
            if (!Array.isArray(wordsToImport)) {
              showError("The imported file does not contain a valid list of words.");
              return;
            }

            // Добавляем только те слова, которые отсутствуют в словаре
            for (const word of wordsToImport) {
              if (!word.englishSpelling || !word.russianSpelling) {
                showError("One or more words are missing essential fields (englishSpelling, russianSpelling).");
                return;
              }

              const isWordPresent = existingWords.some(
                  (existingWord) =>
                      existingWord.englishSpelling === word.englishSpelling &&
                      existingWord.russianSpelling === word.russianSpelling
              );

              if (!isWordPresent) {
                try {
                  console.log(word.russianSpelling);
                  const createdWord = await wordApi.createWord(currentImportDictionaryId, word);

                  // Обновляем состояние
                  const _dictionaries = [...dictionaries];
                  const _index = _dictionaries.indexOf(selectedDictionary);
                  if (_index === -1) {
                    showError("Selected dictionary not found in the dictionaries list.");
                    return;
                  }
                  _dictionaries[_index].words.push(createdWord);
                  ...
                  showError(`Error creating word in the dictionary: ${apiError.message}`);
                  return;
                }
              }
            }

            showSuccess("Words imported successfully");
          } catch (error) {
            showError("Error importing words: " + error.message);
          }
        };
      ...
      showError("Invalid file structure: " + error.message);
    }
  };
    ...
    <div className="dictionary-container">
      ...
      <div className="tables">
        ...
        >
          ...
          <Column header="экспортировать" body={actionDictionaryExport} />
      ...
      <div className="input-area margin-top-30">
        ...
        <div className="word-fields">
          ...
          <Input
            ...
            errorMessage="От 2 до 30 испанских символов"
          />
          <Input
            id="transcription"
